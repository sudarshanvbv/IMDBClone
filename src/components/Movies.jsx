...
function Movies() {
  ...
  function addFavorite(movie) {
    ...
    var watchlist = watchlist ?? JSON.parse(localStorage.getItem("Watchlist"))
    if(watchlist){
    if (watchlist.some((favMov) => favMov.id === movie.id)) {
      console.log("this")
      var newWatchList = watchlist.filter((favMov) => favMov.id !== movie.id)
    } else {
      var newWatchList = [...watchlist, { id: movie.id, image: movie.primaryImage, title: movie.originalTitle,genres:movie.genres,avgRating:movie.averageRating }]
      console.log("watchlist")
    }
  }else{
    console.log("this3")
    var newWatchList = [{ id: movie.id, image: movie.primaryImage, title: movie.originalTitle,genres:movie.genres,avgRating:movie.averageRating }]
  }
    ...
    return (
      <main className="flex flex-wrap justify-evenly">
        {moviesList[pg].map((movie, index) => (
          <section
            key={index}
            className="rounded-lg bg-cover bg-pink-500 bg-center shadow-xl border m-4 h-50 w-40 font-bold flex justify-center items-end hover:scale-110 duration-300 hover:cursor-pointer animate-pulse"
